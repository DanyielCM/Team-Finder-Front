    ...
    fields.forEach((field) => {
      if (!formData[field.name]) {
        ...
        if (/[0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(firstChar)) {
          newErrors[field.name] = `${field.label} cannot begin with a number or special character`;
          valid = false;
        } else if (fieldValue.length < 3) {
          newErrors[field.name] = `${field.label} must be at least 3 characters`;
          valid = false;
        } else if (field.name === 'employeeEmail' && !/\S+@\S+\.\S+/.test(fieldValue)) {
          newErrors[field.name] = 'Invalid email address';
          valid = false;
        } else if (field.name === 'employeePassword') {
          if (fieldValue.length < 5) {
            newErrors[field.name] = 'Password must be at least 5 characters long';
            valid = false;
            ...
            newErrors[field.name] = 'Password must contain at least one special character';
            valid = false;
          }
        }
      }
        ...
        <form onSubmit={handleSubmit}>
          {fields.map((field) => (
            ...
            <div>
              ...
              <div className='error-container'>{errors[field.name] && <div>{errors[field.name]}</div>}</div>
            </div>
          ))}
          <button type="submit" className="form-submit" disabled={!Object.keys(errors).length}>
            Submit
          </button>
        </form>
      </div>
